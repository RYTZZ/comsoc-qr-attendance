...
<head>
    ...
    <title>Verify Code — Computing Society QR Attendance</title>
...
<div class="w-full max-w-md animate-slide-up">
    <div class="text-center mb-8">
        ...
        <h1 class="font-brand-display text-2xl sm:text-3xl font-bold text-white tracking-tight">Enter Verification Code</h1>
        <p class="text-slate-400 mt-1 text-xs sm:text-sm">We sent a 6-digit code to <span class="text-slate-200 font-mono">{{ $email }}</span></p>
    </div>

    <div class="card shadow-xl border border-slate-800/80 bg-[#171a23]">
        <h2 class="font-brand-display text-base font-bold text-white mb-2">Confirm OTP</h2>
        <p class="text-slate-400 text-xs mb-6">Enter the one-time code sent to your registered email address. This code expires in 15 minutes.</p>
        ...
        <form method="POST" action="{{ route('student.activate.verify.submit') }}" class="space-y-4">
            @csrf

            <div>
                <label for="otp" class="label">6-Digit Code</label>
                <input type="text" name="otp" id="otp" inputmode="numeric" pattern="[0-9]{6}" maxlength="6"
                       class="input text-center text-2xl tracking-[0.4em] font-mono py-3 font-bold text-white placeholder-slate-600"
